          ...
          <col width="10%">
            ...
            <th>Action</th>
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT *, full_name as name from `users` order by id desc ");
...
<script>
	$(document).ready(function(){
		$('.delete_data').click(function(){
			_conf("Are you sure to delete this Voter?","delete_user",[$(this).attr('data-id')])
		})
		$('.table').dataTable();
	})
	function delete_user($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/functions.php?f=deletevoter",
			method:"POST",
			data:{id: $id},
			dataType:"json",			
		})
